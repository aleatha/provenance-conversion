...
import time
# used to store the parameters into a python-importable data file
import pickle
#
...
cleanFileName = fileNameM.group(0).rstrip('.txt');
sortedFile = cleanFileName + '_sorted.txt';
pickledFile = cleanFileName + '.pickle';
sortedf = open(sortedFile, 'wt');
#
# regular expressions
restartEx = re.compile(' RESTART ');
timestampEx = re.compile(' TIMESTAMP' );
invalidEx = re.compile(' UID 0 PID 0 ');
decEx = re.compile('\d*');
...
#		sortedf.write('TIMESTAMP!\n\n');
		...
		reps.append(None)
		#time.sleep(0.5);
	elif invalidEx.search(line):
#		sortedf.write('INVALID!\n\n')
		offset.append(None)
		uid.append(None)
		process.append(None)
		abs.append(None)
...
with open(pickledFile, 'wb') as f:
	pickle.dump([offset, uid, pid, process, abs, times, call, rtrn, reps],f)
sortedf.close();
#time.sleep(10);